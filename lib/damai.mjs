...
export function isMobileDamaiUrl(url = '') {
  return /m\.damai\.cn/i.test(url);
}

export function isPcDamaiUrl(url = '') {
  return /detail\.damai\.cn/i.test(url);
}

/** 是否启用移动端抢票（H5 / 回流票监控） */
export function shouldUseMobile(event = {}) {
  if (event.useMobile === false) return false;
  if (event.mobileUrl || event.useMobile === true) return true;
  const url = event.url || '';
  return isMobileDamaiUrl(url) || isPcDamaiUrl(url);
}
...
export function resolveEventUrl(event = {}) {
  ...
  const itemId = extractItemId(event.url);
  if (shouldUseMobile(event) && itemId) {
    return `https://m.damai.cn/shows/item.html?itemId=${itemId}`;
  }

  return event.url;
}

/** 根据 URL 生成 event 配置片段 */
export function eventFromUrl(url, overrides = {}) {
  const itemId = extractItemId(url);
  const useMobile = isMobileDamaiUrl(url) || overrides.useMobile !== false;
  const mobileUrl = itemId
    ? `https://m.damai.cn/shows/item.html?itemId=${itemId}`
    : undefined;
  const pcUrl = itemId
    ? `https://detail.damai.cn/item.htm?id=${itemId}`
    : url;

  return {
    url: isMobileDamaiUrl(url) ? pcUrl : url,
    ...(useMobile && mobileUrl ? { mobileUrl, useMobile: true } : {}),
    ...overrides,
  };
}
